...
const Smartwatch = ({ product }) => {
  ...
  const images = [Smartwatch1, Smartwatch2, Smartwatch3];
  ...
  if (!product) {
    product = {
      ...
      price: 220,
      description:
        "Modern smartwatch with multiple features including heart rate monitoring, GPS, and notifications. Perfect for fitness and daily use.",
    };
  }
    ...
    <div className="flex flex-col lg:flex-row gap-10 p-6 pt-8">
      ...
      <div className="basis-full lg:basis-3/5 flex flex-col items-center lg:items-start mt-24">
        <img
          src={bigImage}
          alt={product.title}
          className={`w-40 sm:w-52 md:w-64 lg:w-full max-h-48 sm:max-h-60 md:max-h-72 lg:max-h-96 rounded-lg shadow-lg mb-4 transition-opacity duration-300 ${
          ...
          {images.map((img, idx) => (
            <img
              ...
              className={`w-20 h-20 sm:w-24 sm:h-24 object-cover rounded-md shadow-md cursor-pointer hover:scale-105 transition-transform duration-200 ${
                bigImage === img ? "ring-2 ring-blue-500" : ""
              }`}
      ...
      <div className="basis-full lg:basis-2/5 flex flex-col justify-center px-4 sm:px-6 lg:px-8">
        ...
        <p className="text-lg sm:text-xl font-semibold text-blue-600 mb-4 text-center">${product.price}</p>
        <button
          onClick={handleAddToCart} 
          className="w-40 px-2 sm:px-3 py-1.5 sm:py-2 bg-blue-500 text-white rounded-md shadow-sm hover:bg-blue-600 transition text-sm sm:text-sm mx-auto"
        >
          Add to Cart
        </button>
      </div>
